...
int main() {
    string fullName, iin, phoneNumber; // Переменные для хранения ФИО, ИИН и номера телефона пользователя
    int regionChoice, placeChoice, subjectChoice; // Переменные для выбора региона, места сдачи и предмета

    // Запрос данных у пользователя
    cout << "Enter your full name: ";
    getline(cin, fullName); // Ввод полного имени (getline используется для считывания всей строки)
    cout << "Enter your IIN: ";
    cin >> iin;
    ...
    // Вывод списка регионов для выбора
    cout << "Choose your region:" << endl;
    for (pair<int, string> &pair : regions) { // Цикл проходит по всем элементам словаря regions
        cout << pair.first << ") " << pair.second << endl;
    }
    cin >> regionChoice;

    // Список мест сдачи экзамена в зависимости от региона
    ...
    for (int i = 0; i < locations[regionChoice].size(); i++) {
        cout << i + 1 << ") " << locations[regionChoice][i] << endl;  // Вывод списка доступных мест
    }
    cin >> placeChoice;

    // Запрос даты и времени сдачи экзамена
    ...
    // Доступные предметные комбинации
